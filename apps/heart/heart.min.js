...
return{isBeat:E.nativeCall(17,"int(int)",a)}}(),HRS={writeByte:function(a,b){I2C1.writeTo(68,a,b)},readByte:function(a){I2C1.writeTo(68,a);return I2C1.readFrom(68,1)[0]},enable:function(){HRS.writeByte(23,16);HRS.writeByte(22,120);HRS.writeByte(1,224);HRS.writeByte(12,110)},disable:function(){HRS.writeByte(1,8);HRS.writeByte(2,128);HRS.writeByte(12,78);HRS.writeByte(22,136);HRS.writeByte(12,34);HRS.writeByte(1,240);HRS.writeByte(12,2);HRS.writeByte(12,34);HRS.writeByte(1,240);HRS.writeByte(12,2);
HRS.writeByte(12,34);HRS.writeByte(1,240);HRS.writeByte(12,2);HRS.writeByte(12,34);HRS.writeByte(1,240);HRS.writeByte(12,2)},read:function(){var a=HRS.readByte(9),b=HRS.readByte(10),c=HRS.readByte(15);return a<<8|(b&15)<<4|c&15}};P8.setLCDTimeout(300);var x=0,lasty=239,lastPeak=0,interval,bpminterval,f1=hpfFilter.filter,f2=medianFilter.filter,f3=agcFilter.filter,f4=lpfFilter.filter,bf=avgMedFilter.filter,det=pulseDetector.isBeat,put=correlator.put;
function doread(){var a=f4(f3(f2(f1(HRS.read()))));a=139-a;a=239<a?239:40>a?40:a;put(a);if(det(a)){var b=Date.now(),c=Math.floor(6E4/(b-lastPeak));0<c&&200>c&&(c=bf(c),g.setColor(-1),g.drawString("BPM: "+c+" ",120,0,!0));lastPeak=b}g.setColor(0);g.fillRect(x,40,x+1,239);g.setColor(2016);g.fillRect(x,lasty,x+1,a);lasty=a;x+=2;240<=x&&(x=0)}function showBPM(){var a=correlator.bpm(),b=correlator.conf();g.setColor(65504).drawString("BPM: "+a+" ",10,0,!0);g.drawString("CNF: "+b+"%  ",10,20,!0)}
